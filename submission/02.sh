# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
TX=""

BLOCKS=$((6*24*7*2))
FUTURE_BLOCK_HEIGHT=$((25 + $BLOCKS))

# Raw transactions argments : 
DECODED_TX=$(bitcoin-cli -regtest decoderawtransaction $TX)
TXID=$(echo $DECODED_TX| jq -r '.txid')



VOUT_1=$(echo $DECODED_TX | jq -r '.vout[0].n')
VOUT_2=$(echo $DECODED_TX | jq -r '.vout[1].n')

INPUTS="[{\"txid\":\"$TXID\",\"vout\":$VOUT_1},{\"txid\":\"$TXID\",\"vout\":$VOUT_2}]"

OUTPUTS="{\"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP\":0.2000000}"

# Create the raw transaction

RAW_TX=$(bitcoin-cli -regtest -named createrawtransaction inputs=$INPUTS outputs=$OUTPUTS locktime=$FUTURE_BLOCK_HEIGHT)

echo $RAW_TX

